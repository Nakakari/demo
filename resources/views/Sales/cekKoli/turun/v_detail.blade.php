            ...
            <div class="ps-3">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0 p-0">
                        ...
                        <li class="breadcrumb-item"><a href="/sales_resi_turun/{{ base64_encode($id_cek) }}">Detail Resi Turun dari Driver {{$cek->driver}}</a></li>
                        <li class="breadcrumb-item active" aria-current="page">
                            Detail Koli Turun dari Resi {{$cek2->no_resi}}
                        </li>
                    </ol>
                </nav>
            </div>
...
@section('js')
    <script type="text/javascript">
        ...
        const table = $("#resi-dt").DataTable({
            ...
            "scrollX": true,
            "ajax": {
                url: "{{ url('/sales_list_detail2/' . base64_encode($id_cek) .'/'. base64_encode($id_pengiriman)) }}",
                type: "POST",
                data: function(d) {
                    d._token = "{{ csrf_token() }}"
                }
            ...
            "columnDefs": [{
                ...
                "render": function(data, type, row, meta) {
                    let dcabang = '';
                    if(data==null){
                        dcabang = "Belum di scan";
                    }else {
                        let cabang = row.nama_kota;
                        dcabang = row.name+' ('+cabang+')';
                    }
                    return dcabang;
                }
            }
