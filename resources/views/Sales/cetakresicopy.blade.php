...
<body onload="window.print();">
    ...
    <table width="100%">
        <tr>
            <td width="10%">
                <img src="{{ url('') }}/img_aplikasi/ase1.png" style="height: 116px" alt="">
            </td>
    ...
    <table width="100%" style="border-collapse:collapse; margin-bottom: 10px">
        <tr style="min-height: 80px; max-height: 80px;">
            <td width="35%" style="padding-left: 12px; padding-right: 2px; vertical-align: top; text-transform: capitalize; text-align: center">
                <b>PENGIRIM :</b> {{ $pengiriman->nama_pengirim }} <br>
                {{ $pengiriman->kota_pengirim }}
                <br>
                {{ $pengiriman->tlp_pengirim }}
            </td>
            <td width="35%" style="vertical-align: top; text-align: center">
                <b>PENERIMA :</b> {{ $pengiriman->nama_penerima }} <br>
            ...
            <td width="30%" style="vertical-align: top; text-align: center">
                <div class="visible-print text-center">
                    {!! QrCode::size(60)->generate(route('pengiriman', base64_encode($pengiriman->dari_cabang)).'?cari_print='.$pengiriman->no_resi); !!}
                </div>
            </td>
        </tr>
    </table>
    ...
    <table width="100%" border="1" style="border-collapse:collapse; margin-bottom: 10px">
        ...
        <tr style="height: 64px">
            ...
            <td style="text-align: center">
                @if ($pengiriman->berat != null)
                    {{ $pengiriman->berat }}
                @elseif ($pengiriman->volume != null)
                    {{ $pengiriman->volume }}
                @else
                    {{ $pengiriman->berat }}{{ $pengiriman->volume }}
                @endif

            </td>
            <td style="text-align: center">
                Rp {{ number_format($pengiriman->biaya, 2, ',', '.') }}
{{--                {{ $pengiriman->biaya }}--}}
            </td>
            <td style="text-align: center">
                {{ $pengiriman->nama_tipe_pemb }}
            </td>
        </tr>
        <tr>
