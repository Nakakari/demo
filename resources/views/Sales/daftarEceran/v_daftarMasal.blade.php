    ...
    @if ($id_cabang == Auth::user()->id_cabang)
        ...
        <div class="card radius-10">
            <div class="card-body">
                ...
                <div class="d-flex align-items-center">
                    <div>
                        <h6 class="mb-2">Daftar Eceran Masal</h6>
                        <div class="col-12">
                            @foreach ($cab as $ca)
                                @if (Auth::user()->id_cabang == $ca->id_cabang)
                                    <p>Daftar eceran masal untuk cabang <b>{{ $ca->nama_kota }}
                                            ({{ $ca->kode_area }})
                                        </b>
                                    </p>
                                @endif
...
@section('js')
    <script type="text/javascript">
        ...
        const table = $("#resi-dt").DataTable({
            ...
            "scrollX": true,
            "ajax": {
                url: "{{ url('/list_masal/' . base64_encode($id_cabang)) }}",
                type: "POST",
                data: function(d) {
                    d._token = "{{ csrf_token() }}"
                }
            ...
            "columnDefs": [{
                ...
                "data": "id_muat_eceran",
                "render": function(data, type, row, meta) {
                    const url_print2 = "/sales_resi_naik/" + btoa(data)
